...
def train_and_select_model(X_train_vec, y_train, config):
    ...
    def _train_model(name, clf_class, params):
        ...
        scoring_metrics = {
            ...
            'f1': 'f1_weighted',
            'roc_auc': 'roc_auc_ovr' if hasattr(clf_class(), 'predict_proba') else None
        }
        ...
        for metric in scoring_metrics:
            if f'mean_test_{metric}' in gs.cv_results_:
                mean_score = gs.cv_results_[f'mean_test_{metric}'][gs.best_index_]
                std_score = gs.cv_results_[f'std_test_{metric}'][gs.best_index_]
                logger.info(f"  {metric.upper()}: {mean_score:.4f} (±{std_score:.4f})")
        
        # Log training vs test scores to check for overfitting
